...
class LinkCog(commands.Cog):
    ...
    @app_commands.command(
        name="link", 
        description="Link your Clash of Clans player tag to your Discord account."
    )
    ...
    async def link(self, interaction: Interaction, player_tag: str):
        ...
        if existing:
            await interaction.followup.send(
                f"**{existing['name']} (#{existing['player_tag']})** is already linked.", 
                ephemeral=True
            )
            return
        ...
        async with aiohttp.ClientSession() as session:
            ...
            if data:
                ...
                await interaction.followup.send(
                    f"✅ Linked {data['name']} (#{tag}) to your account.", ephemeral=True
                )
            else:
                await interaction.followup.send(
                    "❌ Failed to fetch player data. Please check the tag and try again.", ephemeral=True
                )

    @app_commands.command(
        name="unlink", 
        description="Unlink a Clash of Clans tag or all tags linked to your Discord account."
    )
    @app_commands.describe(
        player_tag=(
            "Optional: Specify the player tag to unlink. "
            "If not provided, all linked accounts will be unlinked."
        )
    )
    ...
    @app_commands.command(
        name="linkprofile", 
        description="Show Clash of Clans accounts linked to a Discord user."
    )
    @app_commands.describe(
        user=(
            "Optional: Discord user whose linked Clash accounts you want to view. "
            "Defaults to yourself if not provided."
        )
    )
    async def linkprofile(self, interaction: Interaction, user: Member = None):
        target = user or interaction.user
        linked = get_linked_players_for_user(target.id)
        if not linked:
            await interaction.response.send_message(
                f"No linked accounts found for {target.display_name}.", ephemeral=True
            )
            return
